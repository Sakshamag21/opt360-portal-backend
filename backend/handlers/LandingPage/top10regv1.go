...
// top10RegsCacheTTL is how long a cached top 10 registrars response is considered fresh.
const top10RegsCacheTTL = 10 * time.Minute
...
func GetTop10regV1(c *gin.Context) {
	...
	// ── 2. Cache lookup ────────────────────────────────────────────────────────
	cacheKey := cache.GenerateKey("top10_regs", ro)
	var cached struct {
		Registrars map[string]map[string]int `json:"registrars"`
		RegCodes   map[string]string         `json:"reg_codes"`
	}
	if hit, err := landingCache.Get("top10_regs", cacheKey, top10RegsCacheTTL, &cached); err != nil {
		log.Printf("[GetTop10regV1] Cache read error for ro=%s: %v", ro, err)
	} else if hit {
		log.Printf("[GetTop10regV1] Cache hit for ro=%s", ro)
		c.JSON(http.StatusOK, gin.H{
			"data": gin.H{
				"registrars": cached.Registrars,
				"reg_codes":  cached.RegCodes,
			},
			"regional_office": ro,
		})
		return
	...
	// ── 4. Query raw (reg, reg_code, risk_bucket) counts for the resolved RO ──
	...
	// ── 5. Aggregate reg -> bucket -> count, reg -> code, and reg -> total ─────
	...
	if err := landingCache.Set("top10_regs", cacheKey, cachedData); err != nil {
		log.Printf("[GetTop10regV1] Cache write failed for ro=%s: %v", ro, err)
	}
