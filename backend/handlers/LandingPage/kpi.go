...
// kpiCacheTTL is how long a cached KPI response is considered fresh.
const kpiCacheTTL = 10 * time.Minute
...
func GetKPIData(c *gin.Context) {
	...
	// ── Cache lookup ──────────────────────────────────────────────────────────
	cacheKey := cache.GenerateKey("kpi", ro)
	var cached models.KPIResponse
	if hit, err := landingCache.Get("kpi", cacheKey, kpiCacheTTL, &cached); err != nil {
		log.Printf("[GetKPIData] Cache read error for ro=%s: %v", ro, err)
	} else if hit {
		log.Printf("[GetKPIData] Cache hit for ro=%s", ro)
	...
	// ── Cache write ────────────────────────────────────────────────────────────
	if err := landingCache.Set("kpi", cacheKey, kpiData); err != nil {
		log.Printf("[GetKPIData] Cache write failed for ro=%s: %v", ro, err)
	}
