...
// top10EAsCacheTTL is how long a cached top 10 EAs response is considered fresh.
const top10EAsCacheTTL = 10 * time.Minute
...
func GetTop10EAsV1(c *gin.Context) {
	...
	// ── 2. Cache lookup ────────────────────────────────────────────────────────
	cacheKey := cache.GenerateKey("top10_eas", ro)
	var cached struct {
		Ea     map[string]map[string]int `json:"ea"`
		EaCodes map[string]string         `json:"ea_codes"`
	}
	if hit, err := landingCache.Get("top10_eas", cacheKey, top10EAsCacheTTL, &cached); err != nil {
		log.Printf("[GetTop10EAsV1] Cache read error for ro=%s: %v", ro, err)
	} else if hit {
		log.Printf("[GetTop10EAsV1] Cache hit for ro=%s", ro)
		c.JSON(http.StatusOK, gin.H{
			"data": gin.H{
				"ea":       cached.Ea,
				"ea_codes": cached.EaCodes,
			},
			"regional_office": ro,
		})
		return
	...
	// ── 4. Query raw (ea, ea_code, risk_bucket) counts for the resolved RO ─────
	...
	// ── 5. Aggregate ea -> bucket -> count, ea -> code, and ea -> total ────────
	...
	if err := landingCache.Set("top10_eas", cacheKey, cachedData); err != nil {
		log.Printf("[GetTop10EAsV1] Cache write failed for ro=%s: %v", ro, err)
	}
