...
// maxDateRangeDays bounds how many per-day calls one range search can trigger
// against the SID Store API — it only accepts a single "date" per request, so
// an N-day range means N sequential calls (see fetchSIDsFromAPIDateRange).
const maxDateRangeDays = 31

// fetchAllSIDsForDate pages through fetchSIDsFromAPI's own pagination for one
// date until every record for that date has been collected. batchSize is an
// internal fetch size, unrelated to the caller's own page/page_size — the
// caller paginates over the merged, multi-day result instead (see
// fetchSIDsFromAPIDateRange).
func fetchAllSIDsForDate(optID, date string) ([]PacketResponse, error) {
	const batchSize = 500
	var all []PacketResponse
	page := 1
	for {
		batch, total, err := fetchSIDsFromAPI(optID, date, batchSize, page)
		if err != nil {
			return nil, err
		}
...
// fetchSIDsFromAPIDateRange walks every date in [startDate, endDate]
// (inclusive) and merges each day's full result — the SID Store API has no
// range query of its own, only a single "date" field, so a range search here
// means one fetchAllSIDsForDate call per day.
func fetchSIDsFromAPIDateRange(optID, startDate, endDate string) ([]PacketResponse, error) {
	...
	for _, d := range dates {
		dayRecords, err := fetchAllSIDsForDate(optID, d)
		if err != nil {
			return nil, fmt.Errorf("fetching sids for %s: %w", d, err)
		}
		merged = append(merged, dayRecords...)
	}
	return merged, nil
}

// enumerateDates returns every date (inclusive) between startDate and
// endDate, both "YYYY-MM-DD", in that same format. Capped at
// maxDateRangeDays.
func enumerateDates(startDate, endDate string) ([]string, error) {
	...
	for d := start; !d.After(end); d = d.AddDate(0, 0, 1) {
		if len(dates) >= maxDateRangeDays {
			return nil, fmt.Errorf("date range exceeds max of %d days", maxDateRangeDays)
		}
