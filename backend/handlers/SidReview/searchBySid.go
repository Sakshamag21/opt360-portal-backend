...
func SearchOperatorPacketsBySID(c *gin.Context) {
	...
	// A date range (start_date/end_date) takes priority over the single
	// "date" param when both are somehow given. rangeDates is parsed once
	// here (rather than per-packet in the Parquet filtering loop below) and
	// reused by both the API and Parquet paths.
	isRangeQuery := startDateFilter != "" && endDateFilter != ""
	var rangeDates []string
	if isRangeQuery {
		var err error
		rangeDates, err = enumerateDates(startDateFilter, endDateFilter)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid date range", "details": err.Error()})
			return
		}
	}

	useAPI := searchSID == "" && anomalyFilter == "" && enrollmentTypeFilter == ""

	if useAPI {
		var apiData []PacketResponse
		var totalRecords int
		var err error

		if isRangeQuery {
			log.Printf("[SearchOperatorPacketsBySID] Routing opt_id=%s to SID API (range=%s..%s, %d days, page=%d, page_size=%d)",
				optID, startDateFilter, endDateFilter, len(rangeDates), page, pageSize)
			apiCallStart := time.Now()
			var merged []PacketResponse
			merged, err = fetchSIDsFromAPIDateRange(optID, startDateFilter, endDateFilter)
			log.Printf("[SearchOperatorPacketsBySID] SID API range call for opt_id=%s took %s", optID, time.Since(apiCallStart))

			if err == nil {
				totalRecords = len(merged)
				apiData = paginatePackets(merged, page, pageSize)
			}
		} else {
		...
		if err == nil {
			...
			if isRangeQuery {
				response["date_range_filter"] = gin.H{"start_date": startDateFilter, "end_date": endDateFilter}
			}
	...
	for _, pkt := range mappedData {
		...
		// Filter by date (range takes priority; falls back to single date)
		if isRangeQuery {
			normalizedVal := strings.ReplaceAll(pkt.DateCreated, "-", "_")
			matched := false
			for _, d := range rangeDates {
				if strings.Contains(normalizedVal, strings.ReplaceAll(d, "-", "_")) {
					matched = true
					break
				}
			}
	...
	paginatedData := paginatePackets(filteredData, page, pageSize)
...
// paginatePackets slices data to the given 1-indexed page/pageSize window,
// clamping to bounds instead of panicking when page is past the end.
func paginatePackets(data []PacketResponse, page, pageSize int) []PacketResponse {
	total := len(data)
