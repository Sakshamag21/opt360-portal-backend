...
var (
	...
	portalUsersDB     *sql.DB
	portalUsersDBOnce sync.Once
	portalUsersDBErr  error
)
...
// InitPortalUsersDB initializes the dedicated portal_users database connection
func InitPortalUsersDB(config DBConfig) error {
	portalUsersDBOnce.Do(func() {
		cfg := mysql.Config{
			User:                 config.User,
			Passwd:               config.Password,
			Net:                  "tcp",
		...
		portalUsersDB, portalUsersDBErr = sql.Open("mysql", cfg.FormatDSN())
		if portalUsersDBErr != nil {
			portalUsersDBErr = fmt.Errorf("failed to open portal_users database connection: %w", portalUsersDBErr)
			return
		}

		if err := portalUsersDB.Ping(); err != nil {
			portalUsersDBErr = fmt.Errorf("failed to ping portal_users database: %w", err)
			portalUsersDB.Close()
			portalUsersDB = nil
			return
		}

		applyPoolSettings(portalUsersDB, config)
		log.Println("Portal Users Database connection established successfully")
	})
	return portalUsersDBErr
}

// GetPortalUsersDB returns the dedicated portal_users database connection
func GetPortalUsersDB() (*LoggedDB, error) {
	if portalUsersDB == nil {
		return nil, fmt.Errorf("portal_users database not initialized")
	}
	return newLoggedDB("portal_users", portalUsersDB), nil
}
...
// userLookupTimeout bounds a single portal_users query. The known-slow case
// (~23s under normal load) must still complete within this, so it's set high
// enough to not interfere with that — it exists only to guarantee the call
// eventually returns instead of hanging indefinitely on a genuinely wedged
// connection, so callers waiting on it (see auth.fetchOnce) aren't stuck forever.
const userLookupTimeout = 45 * time.Second

// GetUserByADID retrieves user information from database
func GetUserByADID(adID string) (*models.User, error) {
	database, err := GetPortalUsersDB()
	if err != nil {
	...
	user, err := scanUser(database.QueryRowContext(ctx, query, adID))
...
func updateUserField(column, adID, value string) error {
	...
	if rowsAffected == 0 {
			// MySQL returns 0 rows affected if the row exists but the value was already the same.
			// We treat this as a success since the desired state is achieved.
			log.Printf("Update %s for user_id '%s' resulted in 0 rows affected (value may be unchanged)", column, adID)
			return nil
		}
