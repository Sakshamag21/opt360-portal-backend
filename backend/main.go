...
func main() {
	...
	// Initialize portal database connection (strot_services — anomaly reports)
		...
		// Initialize dedicated portal_users database connection

		if err := db.InitPortalUsersDB(toDBConfig(cfg.Databases.PortalUsers)); err != nil {
			log.Fatal("Failed to initialize portal_users database: ", err)
		}

		fmt.Println("Portal Users Database connection established")
