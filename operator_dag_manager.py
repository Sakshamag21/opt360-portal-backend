...
def _generate_daily_report(**context):
    ...
    try:
        # Airflow 3.0 blocks direct DB ORM access. We must use the REST API.
        ...
        # NOTE: If your Airflow setup requires authentication for the API, 
        # add a token to the headers like this:
        # headers = {"Authorization": f"Bearer {os.getenv('AIRFLOW_API_TOKEN')}"}
        # response = requests.get(api_url, headers=headers)
        
        response = requests.get(api_url)
        response.raise_for_status()
        task_instances_data = response.json().get("task_instances", [])
        
        for ti_inst in task_instances_data:
            tid = ti_inst.get("task_id")
            task_states[tid] = ti_inst.get("state") or "UNKNOWN"
            
            start_str = ti_inst.get("start_date")
            end_str = ti_inst.get("end_date")
            if start_str and end_str:
                # API returns ISO 8601 strings (e.g., '2026-08-17T11:39:15+00:00')
            ...
            task_log_urls[tid] = ti_inst.get("log_url") or "N/A"
