    ...
    with tab1 as(
        ...
        where date(processed_timestamp)>=date('{min_pkt_date}') and date(event_timestamp)>=date('{min_pkt_date}') and date(event_timestamp)< date('{max_pkt_date}') and bio_dev_modality!='Face' and enrl_client_machine_id not like '%SSUP%'
    ),
    tab2 as(
