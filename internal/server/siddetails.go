...
func (s *Server) GetSidDetails(sids []string) response.Response {
	...
	if payload.Status == "OK" {
		rocksdbMap := make(map[string]sidBatchItem, len(payload.Items))
		for _, item := range payload.Items {
			if item.Sid != "" {
				rocksdbMap[item.Sid] = item
			}
		}

		finalSidsInfo := make([]map[string]any, 0, len(sids))
		for _, sid := range sids {
			item, found := rocksdbMap[sid]
			if found && len(item.Values) > 0 {
				values := make(map[string]any, len(item.Values[0])+1)
				for k, v := range item.Values[0] {
					values[k] = v
				}
				values["sid"] = sid
				finalSidsInfo = append(finalSidsInfo, formatItem(values))
			} else {
				finalSidsInfo = append(finalSidsInfo, createNullItem(sid))
			}
		}

		log.Printf("Successfully processed details. Requested: %d, Found: %d. Executed in %s", len(sids), len(payload.Items), time.Since(start))
		return response.Success("SID details retrieved successfully", map[string]any{"sids_info": finalSidsInfo})
	}
