...
type DatabasesConfig struct {
	Portal      DatabaseConfig `json:"portal"`       // strot_services — anomaly reports
	PortalUsers DatabaseConfig `json:"portal_users"` // dedicated DB for opt360-portal-ui user accounts
...
func Load(path string) (*Config, error) {
	...
	applyDatabaseDefaults(&cfg.Databases.Portal)
		applyDatabaseDefaults(&cfg.Databases.PortalUsers)
	...
	if err := validateDatabase("portal", cfg.Databases.Portal); err != nil {
		...
		if err := validateDatabase("portal_users", cfg.Databases.PortalUsers); err != nil {
			return nil, err
		}
...
func applyEnvOverrides(cfg *Config) {
	...
	applyDatabaseEnvOverrides(&cfg.Databases.Portal, "OPT360_DB_PORTAL")
		applyDatabaseEnvOverrides(&cfg.Databases.PortalUsers, "OPT360_DB_PORTAL_USERS")
