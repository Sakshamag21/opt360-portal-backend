...
# ---- ClickHouse serving layer (same connection details as clickhouse_serving_layer/*.py) ----
...
# label -> (table, timestamp column) for the tables each ClickHouse-serving DAG writes to
CLICKHOUSE_TABLES = [
    {"label": "Feature Store", "table": "opt360_feature_store_test_v1", "ts_col": "last_updated_at"},
    {"label": "Risk Analysis", "table": "operator360.risk_analysis", "ts_col": "updated_at"},
    {"label": "Anomalous Packet Store", "table": "operator360.anomalous_packets_v3_dist", "ts_col": "updated_at"},
]

# ---- Airflow REST API (same host referenced in operator_dag_manager.py) ----
AIRFLOW_BASE_URL = os.getenv("AIRFLOW_BASE_URL", "http://10.10.118.36:8080")
AIRFLOW_API_TOKEN = os.getenv("AIRFLOW_API_TOKEN")
AIRFLOW_HEADERS = {"Authorization": f"Bearer {AIRFLOW_API_TOKEN}"} if AIRFLOW_API_TOKEN else {}

# Every DAG that makes up the operator360 pipeline (orchestration, both feature-DAG
# generations, scoring, MySQL/ClickHouse serving, and the standalone jobs) — checked
# for its most recent run so a stuck/failed DAG shows up in the daily report even if
# it never touches Trino/ClickHouse itself.
MONITORED_DAG_IDS = [
    "controller_pipeline", "controller_pipeline_v2",
    "auth_category_features", "auth_category_features_v2",
    "bio_packet_fraud_features", "bio_packet_fraud_features_v2",
    "document_features_combined", "document_features_combined_v2",
    "hardware_category_features", "hardware_category_features_v2",
    "sustxn_category_feature", "sustxn_category_feature_v2",
    "work_category_feature", "work_category_feature_v2",
    "instance_risk_scoring", "instance_risk_scoring_v2",
    "combined_risk_scoring", "combined_risk_scoring_v2",
    "opt_master_updt_dag", "opt360_last_packet_timestamp",
    "opt360_packet_store", "operator_eid_store",
    "opt360_clickhouse_feature_store", "operator_risk_analysis_to_clickhouse",
    "opt360_clickhouse_anomalous_packet_store",
    "opt360_parallel_transactions_eid_updt", "uc_isp_data_enrichment",
    "operator_automated_suspension",
]
...
cc = "sakshamag277@gmail.com,techexe11.yp26@uidai.net.in, techexecutive22-yp23@uidai.net.in"
...
# ---- Airflow 3: timezone-aware start_date, max_active_tasks replaces concurrency ----
default_args = {
    'owner': 'airflow',
...
def get_clickhouse_matrix():
    """Row count + freshness check for every table the ClickHouse-serving DAGs write to
    (opt360_clickhouse_feature_store, operator_risk_analysis_to_clickhouse,
    opt360_clickhouse_anomalous_packet_store)."""
    try:
        ...
        logging.error(f"ClickHouse: could not connect: {e}")
        return pd.DataFrame([
            {"table": t["label"], "rows_yesterday": "N/A", "max_ts": "N/A", "status": f"CONNECT_FAILED: {e}"}
            for t in CLICKHOUSE_TABLES
        ])

    rows = []
    for t in CLICKHOUSE_TABLES:
        row = {"table": t["label"], "rows_yesterday": 0, "max_ts": None, "status": "OK"}
        try:
            result = client.query(f'''
                SELECT count(*) AS cnt, max({t["ts_col"]}) AS max_ts
                FROM {t["table"]}
                WHERE toDate({t["ts_col"]}) = yesterday()
            ''')
            cnt, max_ts = result.result_rows[0]
            row["rows_yesterday"] = cnt
            row["max_ts"] = str(max_ts)
            row["status"] = "OK" if cnt > 0 else "NO DATA YESTERDAY"
        except Exception as e:
            logging.error(f"ClickHouse: query failed for {t['table']}: {e}")
...
def get_airflow_matrix():
    """Latest DagRun state + duration for every DAG in MONITORED_DAG_IDS, via the
    Airflow REST API. For any run that isn't a clean success, also pulls the failed
    task ids so the report can point straight at what broke."""
    rows = []
    for dag_id in MONITORED_DAG_IDS:
        row = {"dag_id": dag_id, "latest_run_state": "UNKNOWN", "logical_date": None,
               "duration_min": None, "failed_tasks": ""}
        try:
            ...
            row["latest_run_state"] = state
            row["logical_date"] = run.get("execution_date")

            start_str, end_str = run.get("start_date"), run.get("end_date")
            if start_str and end_str:
                try:
                    start_dt = datetime.fromisoformat(start_str.replace("Z", "+00:00"))
                    end_dt = datetime.fromisoformat(end_str.replace("Z", "+00:00"))
                    row["duration_min"] = round((end_dt - start_dt).total_seconds() / 60, 1)
                except Exception:
                    pass

            if state != "success" and run_id:
                try:
                    ...
                    failed = [ti["task_id"] for ti in task_instances if ti.get("state") == "failed"]
                    row["failed_tasks"] = ", ".join(failed[:6]) + (" ..." if len(failed) > 6 else "")
                except Exception as e:
                    logging.warning(f"Airflow: could not fetch task instances for {dag_id}/{run_id}: {e}")
                    row["failed_tasks"] = "(could not fetch task instances)"

        except Exception as e:
            logging.error(f"Airflow: could not fetch dagRuns for {dag_id}: {e}")
            row["latest_run_state"] = "UNREACHABLE"
            row["failed_tasks"] = str(e)[:150]
...
def get_executive_summary(sections: dict) -> pd.DataFrame:
    """One-row-per-check top-line status, built from the already-computed section
    DataFrames, so the report opens with a scannable verdict before the detail pages."""
    rows = []

    feature_df = sections.get("Feature Count Overview")
    rows.append({
        "check": "Feature registry (Trino)",
        "status": "OK" if feature_df is not None and not feature_df.empty else "NO DATA",
    })

    instances_df = sections.get("Feature Instances Count")
    if instances_df is not None and not instances_df.empty and "count" in instances_df.columns:
        zero_count = instances_df[instances_df["count"] == 0]
        rows.append({
            "check": "Features with zero rows yesterday",
            "status": f"{len(zero_count)} of {len(instances_df)}",
        })

    ch_df = sections.get("ClickHouse Table Health")
    if ch_df is not None and not ch_df.empty:
        bad = ch_df[ch_df["status"] != "OK"]
        rows.append({
            "check": "ClickHouse serving tables",
            "status": "OK — all fresh" if bad.empty else f"{len(bad)} issue(s): {', '.join(bad['table'])}",
        })

    af_df = sections.get("Airflow DAG Health")
    if af_df is not None and not af_df.empty:
        bad = af_df[~af_df["latest_run_state"].isin(["success"])]
        rows.append({
            "check": "Airflow DAGs (latest run)",
            "status": "OK — all succeeded" if bad.empty else f"{len(bad)} not healthy: {', '.join(bad['dag_id'])}",
        })

    opt_master_df = sections.get("Opt_master MYSQL Table Update")
    rows.append({
        "check": "opt_master MySQL updates",
        "status": "OK" if opt_master_df is not None and not opt_master_df.empty else "NO DATA",
    })

    suspension_df = sections.get("Suspension Signals ")
    rows.append({
        "check": "Suspension signals issued yesterday",
        "status": str(len(suspension_df)) if suspension_df is not None else "N/A",
    })
...
def get_checking_matrix():
    ...
    if 'df' in features_df:
        for i, r in features_df['df'].iterrows():
            ...
            count_id = trino(f'''
                select count(distinct entity_id) as coun from {dest_tab} where feature_id='{feat_id}' and date(timestamp)= date(current_date - interval '1' day)
            ...
            feature_instances_count_df.loc[len(feature_instances_count_df)] = [
                feat_id, dest_tab,
                count_id['df']['coun'].to_list()[0] if 'df' in count_id else 0,
                last_updated_date
            ]
    ...
    last_updated_features_df = trino('''
        select feature_id, destination_table, date(created_at) as created_at, created_by, date(updated_at) as updated_at, updated_by 
        from strot.operator360.opt360_features 
        where date(created_at)= date(current_date - interval '1' day) or date(updated_at)= date(current_date - interval '1' day)
    ''', host='10.10.116.75')
    ...
    clickhouse_matrix_df = get_clickhouse_matrix()
    airflow_matrix_df = get_airflow_matrix()

    sections = {
        "Feature Count Overview": feature_count_df['df'],
        "Feature Instances Count": feature_instances_count_df,
        "ClickHouse Table Health": clickhouse_matrix_df,
        "Airflow DAG Health": airflow_matrix_df,
    ...
    # Executive summary goes first so the PDF opens with a scannable verdict,
    # ahead of the per-check detail pages built above.
    return {"Executive Summary": get_executive_summary(sections), **sections}
...
def trigger_email(query, email_list):
    ...
    html_content1 = (
        f"<p>Please find the attached Operator360 comprehensive system health report for: "
        f"{today_date1}.</p>"
        f"<p>Covers: feature registry (Trino), ClickHouse serving tables, Airflow DAG runs "
        f"(both v1 and v2), Kafka signals/suspensions, and MySQL opt_master updates — see the "
        f"Executive Summary page for the top-line verdict.</p>"
    )
    ...
    emailSubject = f"Operator 360 Comprehensive System Health Report for {today_date}"
    additional_info = {
