import os
os.environ["SPARK_SUBMIT_OPTS"] = "-Dlog4j2.StatusLogger.level=WARN -Dlog4j2.skipUserJars=true"
...
import re
from category_scoring_methods import *
from trino.dbapi import connect
...
def main(
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            print(params['scoring_function'],type(params['scoring_function']))
            scoring_function_name = params.get('scoring_function', '')
            ...
            if scoring_function_name:
                risk_df=scoring_func(spark=spark,risk_weightage=params['risk_weightage'])
            ...
            risk_df= risk_df.select(
                ...
                F.round(F.col("risk_value"),2).cast("double").alias("feature_value"),
        ...
        print(f"Exception in risk score: {e}")
