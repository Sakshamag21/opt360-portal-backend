INSERT INTO {destination_table} 
    with tab1 as (
        ...
        from flink_stream.stream_enu.bi_enu_enrlraw_v2 where date(processed_timestamp)>=date('{min_pkt_date}') and event_timestamp>=date('{min_pkt_date}') and date(event_timestamp)<date('{max_pkt_date}') group by 1 order by dec+approx desc
    )
