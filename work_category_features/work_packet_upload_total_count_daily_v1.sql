insert into {destination_table}
    with tab1 as(
        ...
        from flink_stream.stream_enu.ens_packet_enriched where date(event_timestamp)>= date('{min_pkt_date}') and date(event_timestamp)< date('{max_pkt_date}') and upper(session_operatorid)!='SSUP_OPERATOR' 
    ),
    tab2 as (
