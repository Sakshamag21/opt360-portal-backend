insert into {destination_table}
    with tab1 as(
        ...
        where date(event_timestamp)>= date('{min_pkt_date}') and date(event_timestamp)< date('{max_pkt_date}') and upper(session_operatorid)!='SSUP_OPERATOR' and (enrolment_type='N' or enrolment_type='New') 
    ),
    tab2 as (
        select opt_id, enrolment_eid, event_timestamp from tab1
    ),
    tab3 as (
