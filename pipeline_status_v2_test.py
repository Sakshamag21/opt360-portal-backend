"""Merged status-reporting + idempotency-guard helpers for the layered
operator360 pipeline (controller_pipeline -> Layer 1 feature DAGs ->
Layer 2 instance scoring -> Layer 3 category scoring) - test/v2 variant.

This combines what used to be two separate imports:
...
into one self-contained module, so every _test/_v2 DAG can pull everything
it needs from a single place.

pipeline_status_v2.py is treated as primary: this module points at the test
Ceph S3 config (s3_audit_logger_test, same as pipeline_status_v2.py already
did) rather than the prod config pipeline_status.py used, and it keeps all
of pipeline_status_v2.py's idempotency-guard behavior verbatim. Everything
from pipeline_status.py (topology, status read/write, category verdicts,
report_status, wrap_shell_with_status_report, make_category_gate) is folded
in unchanged so this file has no dependency on pipeline_status.py itself.
...
from operator360.utils.s3_audit_logger_test import (
    CEPH_ACCESS_KEY,
    CEPH_BUCKET_NAME,
    CEPH_CACHE_PREFIX,
    CEPH_ENDPOINT_URL,
...
# instances) avoids that by construction; the functions below are what make
# the same "a task succeeds at most once a day" guarantee hold for Layer 2/3
# as well, where a same-day cascade has to re-trigger a brand-new DagRun
# that necessarily includes categories that already scored - and they're
# also a second, independent line of defense for Layer 1 itself, in case a
# task is ever re-invoked some other way (a manual re-run, Airflow's own
# per-task retries, a future bug).
...
def make_dependency_gate(task_id: str, dependencies: list,
                          run_id_conf_key: str = "pipeline_run_id") -> PythonOperator:
    """Score-level replacement for make_category_gate(): skips unless every
    one of `dependencies` - a list of (dag_id, task_key) tuples for the
    specific upstream feature(s)/score(s) this score actually reads - already
    reported status == "success" for this pipeline_run_id.

    Introduced because make_category_gate() checks a whole category at once
    (every Layer-1 feature in it), but most Layer-2 scores only ever read one
    specific Layer-1 feature, and most Layer-3 category scores only need
    their own category's Layer-2 scores, not an unrelated feature in the same
    category that happens to have failed. Wire it the same way as
    make_category_gate():

        gate = make_dependency_gate(f"gate_dep_{key}", dependencies=[(layer1_dag_id, feature_id)])
        gate >> idempotency_gate

    Falls back to make_category_gate() wherever a score's dependency can't be
    confidently resolved to a single live feature_id (see the DAG file for
    which scores that applies to and why) - this gate is additive, not a
    ...
    def _gate(**context):
        ...
        pipeline_run_id = conf.get(run_id_conf_key) or context.get("run_id")
        missing = [
            f"{dep_dag_id}/{dep_task_key}"
            for dep_dag_id, dep_task_key in dependencies
            if not already_succeeded(pipeline_run_id, dep_dag_id, dep_task_key)
        ]
        if missing:
            logger.info(
                "%s: dependencies not yet succeeded for pipeline_run_id=%s: %s - skipping.",
                task_id, pipeline_run_id, missing,
            )
            raise AirflowSkipException(
                f"Dependencies not yet succeeded for pipeline_run_id={pipeline_run_id}: {missing}"
            )
