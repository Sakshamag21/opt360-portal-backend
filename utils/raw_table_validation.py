...
# TODO: Replace these with your actual Ceph credentials/configs
...
CEPH_CACHE_PREFIX = "cache/airflow/operator360"  # e.g., "airflow/cache/check_raw_tables"
...
def _load_from_s3(s3_client, table_name: str) -> dict:
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'NoSuchKey':
            return None
        print(f"[cache] S3 Error reading {key}: {e}")
        return None
...
def _save_to_s3(s3_client, table_name: str, data: dict):
    ...
    try:
        ...
        print(f"[cache] S3 Error writing {key}: {e}")
...
def check_raw_tables(table_name: str, use_cache: bool = True) -> bool:
    ...
    if use_cache and table_name in _memory_cache:
        ...
        if _is_cache_valid(entry):
            print(f'[cache] MEMORY HIT for {table_name} -> {entry["result"]}')
            return entry['result']
    ...
    if use_cache:
        ...
        if entry and _is_cache_valid(entry):
            print(f'[cache] S3 HIT for {table_name} -> {entry["result"]} (age={int(time.time() - entry["timestamp"])}s)')
            _memory_cache[table_name] = entry  # populate memory for next time in this process
            return entry['result']
        else:
            print(f'[cache] MISS/EXPIRED for {table_name}, querying Trino...')

    # 4. Query Trino
    # Note: Fixed the table_timestamp_column_mapping lookup bug here
    ts_col = table_timestamp_column_mapping[table_name]
    
    # Note: Added `as cnt` to avoid _col0 issues in pandas
    df_result = trino(f'''
        select count(*) as cnt 
        from {table_name} 
    ...
    if 'df' not in df_result.keys():
        print(f'Trino Error: {df_result}')
        return False

    row_count = df_result['df']['cnt'][0]

    if row_count == 0:
        print('No data found of previous date')
        result = False
    else:
        print(f'Data found for previous date in table {table_name}')
        result = True
