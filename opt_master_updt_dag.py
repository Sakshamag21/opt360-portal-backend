...
def update_sync_details():
    ...
    try:
        ...
        # -------- ECMP + UC combined: keep only the most recent sync per opt_id --------
        _log_separator("update_sync_details: combined sync START")
        combined_start = time.time()
        combined_sync = trino('''
            with ecmp as (
                select upper(operator_id) as opt_id, district, pincode, state,
                       cast(latitude as varchar) as latitude, cast(longitude as varchar) as longitude,
                       machine_code, event_timestamp, client_version, client_type
                from flink_stream.stream_enu.enu_operator_sync_raw
                where event_timestamp >= date(current_timestamp - interval '30' day)
            ),
            uc_latest as (
            ...
            uc_agg as (
                select upper(operator_id) as opt_id, resident_sid,
                       max(district) as district, max(pincode) as pincode, max(state) as state,
            ...
            uc as (
                select t1.opt_id, t1.district, t1.pincode, t1.state,
                       cast(t1.latitude as varchar) as latitude, cast(t1.longitude as varchar) as longitude,
                       t1.machine_code, t1.event_timestamp,
                       cast(null as varchar) as client_version,
                       'UC' as client_type
                from uc_agg t1
                right join uc_latest t2 on t1.opt_id = t2.opt_id and t1.resident_sid = t2.resident_sid
                where t2.rw = 1 and t1.opt_id is not null
            ),
            combined as (
                select opt_id, district, pincode, state, latitude, longitude, machine_code,
                       event_timestamp, client_version, client_type
                from ecmp
                union all
                select opt_id, district, pincode, state, latitude, longitude, machine_code,
                       event_timestamp, client_version, client_type
            ...
            ranked as (
                select *, row_number() over (partition by opt_id order by event_timestamp desc) as rw
                from combined
            )
            select opt_id, district, pincode, state, latitude, longitude, machine_code,
                   event_timestamp, client_version, client_type
        ...
        if 'df' in combined_sync and len(combined_sync['df']) > 0:
            df_combined = combined_sync['df']
            logger.info(f"[update_sync_details][COMBINED] Rows fetched (1 per opt_id, latest of ECMP/UC): {len(df_combined)}")
            logger.info(f"[update_sync_details][COMBINED] Source distribution:\n{df_combined['client_type'].value_counts()}")
            success, failed = 0, 0
            for idx, row in df_combined.iterrows():
                combined_upsert_query = f'''
                    ...
                    VALUES
                        ({_sql(row['opt_id'])}, {_sql(row['district'])}, {_sql(row['pincode'])}, {_sql(row['state'])},
                         {_sql(row['latitude'])}, {_sql(row['longitude'])}, {_sql(row['machine_code'])},
                         cast({_sql(row['event_timestamp'])} as datetime), {_sql(row['client_type'])},
                         {_sql(row['client_version'])}, CONVERT_TZ(NOW(), @@session.time_zone, '+05:30'))
                    ON DUPLICATE KEY UPDATE
                ...
                '''
                ok = write_data_to_mysql(combined_upsert_query)
                if ok:
                    success += 1
                else:
                    failed += 1
                if (idx + 1) % 5000 == 0:
                    logger.info(f"[update_sync_details][COMBINED] Progress: processed={idx+1}/{len(df_combined)} "
                                f"success={success} failed={failed}")
            logger.info(f"[update_sync_details][COMBINED] DONE | rows={len(df_combined)} success={success} failed={failed} "
                        f"elapsed={round(time.time()-combined_start,3)}s")
        else:
            logger.warning("[update_sync_details][COMBINED] No data returned from Trino.")
...
def update_operator_sync_location():
    ...
    try:
        # -------- ECMP + UC combined: keep only the most recent sync per opt_id --------
        _log_separator("update_operator_sync_location: combined sync START")
        combined_start = time.time()
        combined_sync = trino('''
            with ecmp as (
                select upper(operator_id) as operator_id,
                       cast(latitude as varchar) as latitude, cast(longitude as varchar) as longitude,
                       event_timestamp as last_updated_date
                from flink_stream.stream_enu.enu_operator_sync_raw
            ...
            uc as (
                ...
                from uc_agg t1
                right join uc_latest t2 on t1.operator_id = t2.operator_id and t1.resident_sid = t2.resident_sid
                where t2.rw = 1 and t1.operator_id is not null
            ),
            combined as (
                select operator_id, latitude, longitude, last_updated_date from ecmp
                union all
                select operator_id, latitude, longitude, last_updated_date from uc
            ),
            ranked as (
                select *, row_number() over (partition by operator_id order by last_updated_date desc) as rw
                from combined
            )
            select operator_id, latitude, longitude, last_updated_date
            from ranked
            where rw = 1
        ...
        if 'df' in combined_sync and len(combined_sync['df']) > 0:
            loc_data = combined_sync['df']
            logger.info(f"[update_operator_sync_location][COMBINED] Rows fetched (1 per opt_id, latest of ECMP/UC): {len(loc_data)}")
            ...
            logger.info(f"[update_operator_sync_location][COMBINED] Prepared {len(data_to_insert)} valid rows. Skipped {len(skipped_rows)} invalid rows due to bad GPS data.")
                ...
                loc_upsert_query = f'''
                    ...
                    VALUES
                        ({_sql(operator_id)},
                         ...
                         cast({_sql(updt_str)} as datetime))
                    ON DUPLICATE KEY UPDATE
                        loc = VALUES(loc),
                        updated_at = VALUES(updated_at)
                '''
                ok = write_data_to_mysql(loc_upsert_query)
                if ok:
                    success += 1
                else:
                    failed += 1
                if (idx + 1) % 5000 == 0:
                    logger.info(f"[update_operator_sync_location][COMBINED] Progress: processed={idx+1}/{len(data_to_insert)} "
                                f"success={success} failed={failed}")
            logger.info(f"[update_operator_sync_location][COMBINED] DONE | rows={len(data_to_insert)} success={success} failed={failed} "
                        f"elapsed={round(time.time()-combined_start,3)}s")
        else:
            logger.warning("[update_operator_sync_location][COMBINED] No data returned from Trino.")
